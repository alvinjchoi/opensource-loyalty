---
title: "Migrating a self-hosted LIP program"
description: "A zero-loss runbook for moving one LIP program between SQLite and Postgres hosts without losing member or financial state."
...
<Info>
The published container (available on npm under the `@loyalty-interchange` org) can run the same commands with `node packages/cli/dist/cli.js` in place of `npm run lip --`.
</Info>
